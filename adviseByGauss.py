...
def analyze_stock(stock_name, stock_code, stock_type, revenue_per_share_yoy, price_data, revenue_per_share,
    ...
    # 设置核函数的参数范围，降低 length_scale 的下限
    param_grid = {
        'kernel': [RationalQuadratic(length_scale=l, alpha=a) + RBF(length_scale=l)
                   for l in [1e-07, 1e-06, 1e-05, 1e-04, 1e-03, 0.01, 0.1, 1.0, 10.0]
                   for a in [0.1, 1.0]]
    }
