...
def analyze_stock(stock_name, stock_code, stock_type, revenue_per_share_yoy, price_data, revenue_per_share,
    ...
    # 使用岭回归模型
    ridge_regression = make_pipeline(PolynomialFeatures(degree=1), Ridge())

    # 拟合模型
    ridge_regression.fit(X_train, y_train)

    # 预测和评估
    y_pred_final = ridge_regression.predict(X_test)
    ...
    estimated_price_scaled = ridge_regression.predict(current_feature_scaled)
    ...
    predicted_price = ridge_regression.predict(combined_features_all)
