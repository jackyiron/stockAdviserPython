...
def analyze_stock(stock_name, stock_code, stock_type, revenue_per_share_yoy, price_data, revenue_per_share,
    ...
    svr = SVR()
    parameters = {'C': [0.1, 1.0, 10.0], 'epsilon': [0.01, 0.1, 0.2]}  # 你可以根据需要调整搜索的参数范围
    grid_search = GridSearchCV(svr, parameters, scoring='neg_mean_squared_error', cv=5)
