...
def analyze_stock(stock_name, stock_code, stock_type, revenue_per_share_yoy, price_data, revenue_per_share,
    ...
    # 划分训练集和测试集
    X_train, X_test, y_train, y_test = train_test_split(X_poly, price_normalized.flatten(), test_size=0.2, random_state=42)
    ...
    current_feature_poly = poly.transform(current_feature_scaled)
    estimated_price_scaled = ridge.predict(current_feature_poly)
    ...
    predicted_price = ridge.predict(X_poly)
